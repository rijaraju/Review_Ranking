# Datapipeline for the review ranking deployement
...
parser.add_argument(
    ...
    type=int,
    default=0.9,
    help="Spell checking threshold",
)
parser.add_argument(
    "--model_path",
    action="store",
    type=str,
    default="randomforest.joblib",
    help="Model Path",
)
parser.add_argument(
    "--file_name",
...
parser.add_argument(
    ...
    type=str,
    default="False",
    help="Get Ranking Score Test or Not (True/False)",
)
...
for product in product_list:
    ...
    df.loc[df["product"] == product, "Rn"] = rf.noun_score(
        data["answer_option"].values
    ).values
...
for i, product in enumerate(product_list):
    ...
    for indx in data.index:
        ...
        df.at[indx, "review_score"] = (
            float(0 if score.get(1) is None else score.get(1)) / len(data) * 1.0
        )
...
if args.testing == "True":
    ...
    for product in product_list:
        ...
        number_of_1_in_x = Counter(df[df["product"] == product].iloc[:x,]["label"]).get(
            1
        )
...
df[["product", "answer_option", "review_score"]].to_csv(
    "data/test_ranked_output.csv", index=False
)
